...
imdir='ELISA catrige data';
if ~exist(fullfile(imdir, '8.0mm', 'bb', 'avg.png'), 'file') 
    averagePhotos(fullfile(imdir, '8.0mm', 'bb'), 8);
end
I = im2double(imread(fullfile(imdir, '8.0mm', 'bb', 'avg.png')));
left_off = 1100;
right_off = 1100;
...
dirs = {'4.0mm', '4.5mm', '5.0mm', '5.5mm', '6.0mm', '6.5mm', '7.0mm', '7.5mm'};
figure;
for i=1:length(dirs) 
    if ~exist(fullfile(imdir, char(dirs(i)), 'bb', 'avg.png'), 'file')
        averagePhotos(fullfile(imdir, char(dirs(i)), 'bb'), 8);
    end
    I2 = im2double(imread(fullfile(imdir, char(dirs(i)), 'bb', 'avg.png')));
    ...
    %figure;
    s(s >= 1) = 1;
    plot(nm, s, 'LineWidth', 1.5);
    hold on;
end
plot(nm, bg, 'LineWidth', 1.5, 'Color', 'k');
legend('4.0mm', '4.5mm', '5.0mm', '5.5mm', '6.0mm', '6.5mm', '7.0mm', '7.5mm', '8.0mm')
xlim([390, 670])
ylim([0, 1.01])
xlabel('Wave length')
...
dirs = {'8.5mm', '9.0mm', '9.5mm', '10.0mm', '10.5mm', '11.0mm', '11.5mm', '12.0mm'};
figure;
for i=1:length(dirs) 
    if ~exist(fullfile(imdir, char(dirs(i)), 'bb', 'avg.png'), 'file')
        averagePhotos(fullfile(imdir, char(dirs(i)), 'bb'), 8);
...
plot(nm, bg, 'LineWidth', 1.5, 'Color', 'k');
legend('8.5mm', '9.0mm', '9.5mm', '10.0mm', '10.5mm', '11.0mm', '11.5mm', '12.0mm', '8.0mm')
xlim([390, 670])
ylim([0, 1.01])
xlabel('Wave length')
ylabel('Normalized intensity');